...
var LogUtil = module.exports = {
  ...
  cantGet: function(topic, paramsOrId, errorOrResponseStatusCode, url) {
    ...
    if(paramsOrId == null) output += '(no params)';
    else if(typeof(paramsOrId) == 'string') output += paramsOrId;
    else output += LogUtil.paramString(paramsOrId);
    
    output += '. ';
    
    
    if(isNaN(errorOrResponseStatusCode)) output += String(errorOrResponseStatusCode);
