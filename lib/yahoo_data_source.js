var http = require('http'),
    ...
    flow = require('nimble'),
    csv = require('ya-csv'),
    time = /true/i.test(process.env.CANNED_DATA) ? null : require('time'),
...
function YahooDataSource(callback) {
  var date = time ? new time.Date() : null;
  if (date) date.setTimezone('America/New_York');
  
  var nycTimeShifted = function(t) { return date ? (date.setTime(t), t - date.getTimezoneOffset() * 60000) : t; },
  ...
  this.getTimeSeries = function(params, callback) {
    // If using canned data mode, try to load from cache files
    if(/true/i.test(process.env.CANNED_DATA)) {
      ...
      if(fs.existsSync(cacheFile)) {
        ...
        if(verbose) console.log('Cache file not found:', cacheFile);
        callback(null);
        return;
      }
    }
    
    // Original live data fetching
    switch(params.type) {
      case 'intraday':
  ...
  this.getNews = function(params, callback) {
    if(/true/i.test(process.env.CANNED_DATA)) {
      // Try to load news from cache
      var newsFile = __dirname + '/../public/data/' + process.env.DATA_DOMAIN + '/news_cache/' + params.id + '_news.json';
      
      if(fs.existsSync(newsFile)) {
        fs.readFile(newsFile, 'utf8', function(err, data) {
          if(err) {
            console.error('Error reading news cache:', newsFile, err);
            callback([]);
          } else {
            try {
              callback(JSON.parse(data));
            } catch(e) {
              console.error('Error parsing news cache:', newsFile, e);
              callback([]);
            }
          }
        });
      } else {
        // No news cache for this stock
        callback([]);
      }
      return;
    }
    ...
    // The following includes a hot fix (tagged YAHOO BUGFIX) for a bug in the Yahoo Finance RSS feed,
    // which at the time of this writing fails to wrap the response in an RSS tag,
    // which breaks the parser.
    // This fixes the problem by injecting the enclosing RSS tag prior to and after streaming the
    // http response. It was tested and verified that if Yahoo introduces a fix, this hotfix should
    // not break things, because feedParser can deal with the double <rss> tags.

    feedParser.write('<rss version="2.0">') /* YAHOO BUGFIX */
    ...
    request(url)
      ...
      .on('end', function() {
        feedParser.write('</rss>') /* YAHOO BUGFIX */
      })
      .pipe(feedParser)
      .on('error', function(error) {
        console.error( "Can't parse news: " + url );
        // return callback([]); // commented out, because turns out 'end' event occurs anyway
      })
  ...
  function requestAllQuotes() {
    ...
    for(var i = 0; i < symbols.length; i += 50) {
      requests.push(requestFunction(symbols.slice(i, i+50)));
    }
  ...
  function requestFunction(symbols) {
    return function(cb) {
      setTimeout(function() {// RATE LIMITING. COMMENT OUT IF NOT NEEDED
        var url = 'http://download.finance.yahoo.com/d/quotes.csv?s=' + symbols.join(',') + '&f=sl1d1t1c1pohgvp2j1a2jk';
      ...
      }, 1000);// RATE LIMITING. COMMENT OUT IF NOT NEEDED
    };
  }
}