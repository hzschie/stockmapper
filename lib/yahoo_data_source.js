    ...
    request(
      ...
      function(error, response, body) {
        if(error || response.statusCode >= 400) {
          ...
          else console.error("Can't get chart data for s=" + id + ". Response status code is " + response.statusCode);
          return;
        }
        ...
        reader.on('data', function(row) {
          series.addSlice( [Date.parse(row.Date), parseFloat(row.Close), parseFloat(row.Volume)] );
        });
        reader.parse(body);
        callback(series);
      }
