...
module.exports = (function() {
  ...
  var verbose = /true/i.test(process.env.VERBOSE),
      urlBase = 'https://' + process.env.DATA_HOST + '/Service/equities.svc/',
      groupsTable,
      groups,
      stocks,
      cursor = null;
  ...
  function createGroups(callback) {
    ...
    var url = urlBase + 'GetEntireIndexList';//'GetBlufinIndexList';
    var requestOpts = {
      url: url,
      headers: {
    ...
    request(requestOpts, function(error, response, body) {
      if(error || response.statusCode >= 400) {
        ...
        console.error( LogUtil.cantGet('GetEntireIndexList', null, error || response.statusCode, url) );
        return callback();
      }

      var groupsJson = JSON.parse(body).filter(function(group) { return group.dp > 0; });
      ...
      groupsJson.forEach(function(group, i) {
        ...
        groups.push(
          groupsTable[groupId] = {
            ...
            nickname: group.n.replace(/ and /, ' & '),//.replace(/^blufin /i, '').replace(/ index$/i, '')
            category: group.c,// Category
            ids: [],
            resourceParams: 'resource=index&' + (group.s ? group.s.toLowerCase() + '=true' : '') //eg: 'resource=index&blufin=true'
          }
        );
      });
...
*/
      ...
      groups.sort(function(a, b) { return (a.dp > b.dp) - (b.dp > a.dp); });
  ...
  function createStocks(callback) {
    var url = urlBase + 'GetLatestIndexConstituentsDataByIndexID?IndexID=' + ALL_GROUP_ID;
    var requestOpts = {
      url: url,
      headers: {
        Referer: "https://www.blufinresearch.com/"
    ...
    request(requestOpts, function(error, response, body) {
      if(error || response.statusCode >= 400) {
        cursor.hex('#cc0000').write('');
        console.error( LogUtil.cantGet('index constituents', 'IndexId=' + ALL_GROUP_ID, error || response.statusCode, url) );
        return callback();
      }

      var stocksRaw = JSON.parse(body),
          broadGroup = groupsTable[ALL_GROUP_ID],
          niftyGroup = groupsTable[NIFTY_GROUP_ID],
          sensexGroup = groupsTable[SENSEX_GROUP_ID];
      ...
      stocksRaw.forEach(function(stock) {
        ...
        stocks.push( [stock.cid, stock.n, sym, 'resource=stock&nse=' + (stock.nse == 1)] );//[ScripCode, ScripName, sym, resourceParams]
        ...
        for(var indexId in groupsTable) {
          ...
          if(group.name == sector || group.name == capitalization || group.name == style || group.name == crosstab || 
              (group == broadGroup && !(!stock.c && !stock.sec && !stock.st)) || 
              (isNifty && group == niftyGroup) || (isSensex && group == sensexGroup)) {
