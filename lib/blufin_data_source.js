...
function BlufinDataSource(dataCallback) {
  ...
  this.getExtendedDataset = function(name, response) {
    if(name == 'low_high') {
      if(process.env.CANNED)   {
        response.end(fs.readFileSync(__dirname + '/../public/data/blufin/dummy_low_high.json', 'utf8'));
        return;
      }
  ...
  function requestAll() {
    if(process.env.CANNED) {
      setTimeout(function() {
        dataCallback(JSON.parse(fs.readFileSync(__dirname + '/../public/data/blufin/dummy_stocks.json', 'utf8') ).map(function(data) { return table[data[1]] = data; }));
        /* // TO MAKE:
        JSON.stringify(
          mapper.stocks.map(function(s) { return mapper.Stock.fields.map(function(f,i) { if(i==0)return 'stock';if(i==1)return s.id; return s.get(f.name); }); })
        );
        */
      }, 0);
      return;
