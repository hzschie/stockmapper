...
function BlufinDataSource(dataCallback) {
    ...
    request(
      ...
      function(error, response, body) {
        if(error || response.statusCode >= 400) {
          ...
          else console.error("Can't get news for query=" + params.id + ". Response status code is " + response.statusCode + '. Url: ' + url);
          return callback([]);
        }
  ...
  function getIntraday(params, callback) {
    ...
    request(
      ...
      function(error, response, body) {
        if(error || response.statusCode >= 400) {
          if(error) console.error(error);
          else console.error("Can't get intraday data for Key=" + id + ". Response status code is " + response.statusCode + '. Url: ' + url);
  ...
  function get5day(params, callback) {
    ...
    flow.parallel(
      ...
      function() {
        callback(_4d ? _4d.concat(intraday) : intraday);
      }
    );
  };
  ...
  function requestLowHighDataset() {
    console.log('Acquiring LowHighDataset... at UTC ' + (new Date().toUTCString()));
    ...
    request(
      ...
      function(error, response, body) {
        if(error || response.statusCode >= 400) {
          if(error) console.error(error);
          else console.error("Can't get lows & highs. Response status code is " + response.statusCode + '. Url: ' + url);
          return;
        }
        
        cachedLowHighDataset = body;
        console.log('LowHighDataset acquired successfully!');
      }
    );
  }
  
  function requestAllQuotes() {
    console.log('Acquiring all quotes... at UTC ' + (new Date().toUTCString()));
    var cachedRows = [];
    flow.parallel(
      [
        function(flowCallback) {
          ...
          request(
            ...
            function(error, response, body) {
              if(error || response.statusCode >= 400) {
                if(error) console.error(error);
                else console.error("Can't get index constituents for indexId=1000. Response status code is " + response.statusCode + '. Url: ' + url);
                return;
              }
        ...
        function(flowCallback) {
          ...
          request(
            ...
            function(error, response, body) {
              if(error || response.statusCode >= 400) {
                if(error) console.error(error);
                else console.error("Can't get index data for indexId=0. Response status code is " + response.statusCode + '. Url: ' + url);
                return;
              }
