    ...
    request(
      ...
      function(error, response, body) {
        if(error || response.statusCode >= 400) {
          ...
          else console.error("Can't get news for Key=" + params.id + ". Response status code is " + response.statusCode);
          return callback([]);
        }
  ...
  function get5day(params, callback) {
    ...
    flow.parallel(
      [
        ...
        function(cb) { _this.getIntraday(params, function(series) { intraday = series; cb(); }); }
      ], 
      function() {
