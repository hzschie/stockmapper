...
function BlufinDataSource(dataCallback) {
  ...
  this.getNews = function(params, callback) {
    request(
      BlufinDataSource.urlBase + 'News.svc/GetLatestNews?query=' + params.id,
      function(error, response, body) {
        if(error || response.statusCode >= 400) {
          if(error) console.error(error);
          else console.error("Can't get news for query=" + params.id + ". Response status code is " + response.statusCode);
          return callback([]);
        }
  ...
  function getIntraday(params, callback) {
    ...
    else url += 'GetIntrdayData?ExchangeID=BSE&Key=' + id;
    request(
      url,
      function(error, response, body) {
        if(error || response.statusCode >= 400) {
  ...
  function getCsvTimeSeries(params, type, callback) {
    var id = params.id,
        url = 'http://46.137.212.140/BMI/data/' + id + (/index/i.test(params.resource) ? '_idx' : '') + '_bse_' + type + '.csv';
    request(
      url,
      function(error, response, body) {
        if(error || response.statusCode >= 400) {
