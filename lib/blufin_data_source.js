...
function BlufinDataSource(dataCallback) {
  ...
  this.getDaily = function(id, callback) {
    request(
      'http://46.137.212.140/BMI/data/' + id + '_bse_his.csv',
      function(error, response, body) {
        if(error || response.statusCode >= 400) {
          if(error) console.error(error);
        ...
        reader.on('data', function(row) {
          var parts = row.TDate.match(/(\d+)/g);
          series.addSlice( [ Date.UTC(parts[2], parts[0]-1, parts[1]), parseFloat(row.CurrentPrice), parseFloat(row.Volume) ] );
        });
        reader.parse(body);
...
/*
    ...
    return table[id] = [
      'stock', id, lastTrade, lastTradeDate, lastTradeTime, change, previous, open, high, low, volume, changePct, marketCapString, avgVolume];
*/
  }
