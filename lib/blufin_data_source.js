...
function BlufinDataSource(dataCallback) {
  ...
  this.getDataset = function(name, response) {
    switch(name) {
      ...
      case 'low_high': {
        request(BlufinDataSource.urlBase + '/equities.svc/GetIndexConstiuentsHighLow?IndexID=1000').pipe(response);
        break;
      }
    }
  };
  ...
  function getCsvTimeSeries(params, type, callback) {
    var id = params.id,
        // url = 'http://' + process.env.DATA_HOST + '/BMI/data/' + id;// Change to this line once CSVs have been migrated
