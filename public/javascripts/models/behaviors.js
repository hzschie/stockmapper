(function() {
  ...
  mapper.GroupBehaviors.keepCounts = function(group) {
    ...
    if(members.length) group.updateCounts();
  };

  mapper.GroupBehaviors.pickMembers = function(group, n) {
    var updatePending = false;
    ...
    members.on('change:volume change:changePct', function(model) {
      updatePicks();
    });
    
    if(members.length) updatePicks();
  };
})();