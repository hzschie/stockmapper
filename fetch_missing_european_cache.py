#!/usr/bin/env python3
"""
Fetch cache data for European stocks that are missing cache files
"""
...
def main():
    # Read European stocks from stk.csv
    df = pd.read_csv(STK_FILE, skiprows=1)
    
    # European stock suffixes
    eu_suffixes = ['.L', '.PA', '.DE', '.AS', '.MC', '.MI', '.BR', '.VX']
    eu_stocks = df[df['TICKER'].str.contains('|'.join([s.replace('.', '\\.') for s in eu_suffixes]), na=False, regex=True)]
    
    # Check which are missing cache
    cache_files = set(os.listdir(CACHE_DIR))
    missing = []
    
    for ticker in eu_stocks['TICKER']:
        if f"{ticker}_daily.json" not in cache_files:
            missing.append(ticker)
    
    print(f"Found {len(missing)} European stocks without cache data")
    
    if not missing:
        print("All European stocks already have cache data!")
        return
